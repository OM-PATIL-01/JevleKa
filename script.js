...
function playOrderConfirmationSound() {
    console.log("Attempting to play order confirmation sound: yehle.mp3");
    const audio = new Audio('yehle.mp3');
    audio.play().then(() => {
        console.log("Confirmation sound played successfully.");
    }).catch(err => {
        console.error("Error playing sound:", err);
        // Sometimes browsers block audio until FIRST interaction. 
        // We already have interaction here, but let's log it clearly.
    });
}
...
function openTicketModal(id, fullCode, method) {
    ...
    const user = JSON.parse(localStorage.getItem('user')) || {};
    const qrData = `Order: ${fullCode}|User: ${user.username}|Time: ${new Date().toLocaleString()}`;
    const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(qrData)}`;
